...
2. **学习者作答**：在测试题文件中直接回答；
3. **批改过程**：
   - 每次提交后，Claude 分析每道题的问题；
   - 记录错误原因、概念误区、需要补强的点；
   - 如果未通过，给出针对性的补强练习或复测题；
4. **通过后生成**：
   - 在对应章节目录下生成 `学习报告.md`，包含：
     - 每次提交的问题分析
     - 最终通过的完整题目和正确答案
     - 薄弱点总结
     - 进步轨迹
   - 自动调用 `/stage-passed {阶段名}` 提交代码；
5. **复测题**：如果多次未通过，生成针对性的复测题（更简洁、聚焦核心问题）。
...
├── 复测题/                   # 如果需要复测（无答案）
│   └── 复测题.py
└── 学习报告.md               # 通过后生成（包含问题分析和最终答案）
```

**已迁移到中文结构：**
- `Python-阶段03-面向对象与异步/` — 整合自 python-stage-03 和 python-stage-03-practice
